globals [
  ...
  sun-brightness ;; changing the sun-brightness slider to a global fixed value between 0 and 5 (step = 0.2)
  albedo ;; changing the albedo slider to a global fixed value between 0 and 1 (step = 0.05)
  cloud-coverage-base ;; enables checking the variable on the go
  CO2-amount-base ;; enables checking the variable on the go
]
...
to setup-world
  ...
  setup-world-updated ;; Elham's updates
...
to setup-world-updated
  set sun-brightness 1.0
  set albedo 0.6
  set CO2-amount-base CO2-amount
  set cloud-coverage-base cloud-coverage
  
  repeat ( CO2-amount / 25 ) [add-CO2] 
  repeat ( cloud-coverage / 5 ) [ add-cloud ] 
...
to go-updated
  ;; check the CO2 slider
  ifelse ( CO2-amount >= CO2-amount-base ) 
  [ 
    let CO2-diff CO2-amount - CO2-amount-base
    repeat ( CO2-diff / 25 ) [add-CO2] 
    set CO2-amount-base CO2-amount
  ]
  [ 
    let CO2-diff CO2-amount-base - CO2-amount
    repeat ( CO2-diff / 25 ) [remove-CO2] 
    set CO2-amount-base CO2-amount
  ]
  
  ;; check the cloud-coverage slider
  ifelse ( cloud-coverage >= cloud-coverage-base ) 
  [ 
    let cloud-diff cloud-coverage - cloud-coverage-base
    repeat ( cloud-diff / 5 ) [add-cloud] 
    set cloud-coverage-base cloud-coverage
  ]
  [ 
    let cloud-diff cloud-coverage-base - cloud-coverage
    repeat ( cloud-diff / 5 ) [remove-cloud] 
    set cloud-coverage-base cloud-coverage
  ]
